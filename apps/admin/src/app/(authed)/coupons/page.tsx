  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-8">
      <div className="flex flex-wrap items-baseline justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Coupon code usage in the period. Auto cart-rule promotions (no code) reported
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Tile
          label="Coupon orders"
          value={formatNumber(result.totals.coupon_orders)}
          accent="primary"
        />
        <Tile
          label="Coupon revenue"
          value={formatCurrencyArs(result.totals.coupon_revenue)}
          sub="post-discount totals"
          accent="success"
        />
        <Tile
          label="Discount given"
          value={formatCurrencyArs(result.totals.discount_total)}
          sub="across coupons"
          accent="destructive"
        />
      </div>
        ...
        <table className="w-full text-left text-sm">
          <thead className="border-b border-border bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground">
            <tr>
              ...
              <th className="px-4 py-3 text-right font-semibold">Net (real)</th>
            ...
            {result.data.length === 0 && (
              <tr>
                <td colSpan={8} className="px-4 py-10 text-center text-muted-foreground">
                  No coupon codes used in this window. As soon as a customer applies one in
                  Magento, it&apos;ll show up here.
                </td>
              </tr>
            )}
            {result.data.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="relative px-4 py-3 text-right">
                    <div className="relative inline-flex items-center justify-end gap-2">
                      <span className="tabular-nums font-medium text-foreground">
                        {formatCurrencyArs(c.gross_revenue)}
                      </span>
                    </div>
                    <span
                      aria-hidden="true"
                  ...
                  <td className="px-4 py-3 text-right tabular-nums text-destructive">
                    −{formatCurrencyArs(c.discount_total)}
                  </td>
                  <td className="px-4 py-3 text-right tabular-nums text-foreground/80">
                    {formatCurrencyArs(c.net_revenue)}
                  </td>
                  <td className="px-4 py-3 text-xs text-muted-foreground">
                    {formatBuenosAires(c.last_used_at)}
                  </td>
                </tr>
              );
            })}
      ...
      {result.totals.auto_promo_orders > 0 && (
        <section className="rounded-lg border border-border bg-card p-5 shadow-card">
          <h2 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
            Automatic cart rules (no coupon code)
          </h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Orders that received a discount in the period without a coupon being typed in. Useful
            to compare promo-driven vs coupon-driven revenue.
          </p>
          <dl className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="rounded-md border border-border bg-background/50 p-3">
              <dt className="text-xs text-muted-foreground">Orders with auto promo</dt>
              <dd className="mt-1 text-lg font-semibold tabular-nums text-foreground">
                {formatNumber(result.totals.auto_promo_orders)}
              </dd>
            </div>
            <div className="rounded-md border border-border bg-background/50 p-3">
              <dt className="text-xs text-muted-foreground">Discount given (auto)</dt>
              <dd className="mt-1 text-lg font-semibold tabular-nums text-destructive">
                −{formatCurrencyArs(result.totals.auto_promo_discount)}
              </dd>
            </div>
          </dl>
        </section>
