# Datapp — Production deploy runbook

> Step-by-step procedure to bring a fresh Datapp install up on a Hostinger VPS through Dokploy with Cloudflare in front. Pair this file with `docker-compose.dokploy.yml`, `infra/docker/*`, and `infra/backups/`.
...
- A Sentry project for both `datapp-api` and `datapp-admin`.
...
- [ ] Create the box, log in as `root` once, then **immediately** make a sudo user:
  ...
  ```sh
  adduser datapp
  usermod -aG sudo datapp
  rsync --archive --chown=datapp:datapp ~/.ssh /home/datapp
  ```

- [ ] Lock down sshd (`/etc/ssh/sshd_config.d/99-datapp.conf`):
  ...
  **Verify in a separate terminal** that `ssh datapp@host` still works before closing the original session.
...
- [ ] Open the Dokploy UI on `https://<vps-ip>:3000` (one-time bootstrap), create the admin user, then put it behind a Cloudflare-proxied subdomain (e.g. `dokploy.datapp.com.ar`). Don't leave the panel exposed on a public IP+port.
...
- [ ] Add A records (proxied) for:
  ...
  ```
  api.datapp.com.ar    → <vps-ip>
  datapp.com.ar  → <vps-ip>
  ```
...
- [ ] Generate an **Origin Certificate** (SSL/TLS → Origin Server) for `*.datapp.com.ar`. Copy the cert + private key. In Dokploy, paste them into the Traefik certs panel for both subdomains so the origin actually serves TLS.

- [ ] In **Rules → Configuration Rules**, set:

  - Browser Cache TTL = "Respect existing headers" for `api.datapp.com.ar`.
  - Cache Level = "Bypass" for `api.datapp.com.ar` (the API responses are dynamic).

- [ ] In **Speed → Optimization**, **disable** Auto Minify (breaks Next.js script hashes) and Rocket Loader.

- [ ] Optional but recommended: enable **WAF managed rules** for `datapp.com.ar` (paid feature on Pro+).
...
- [ ] In [Google Cloud Console](https://console.cloud.google.com), create a project named `datapp-prod` (or reuse an existing one).

- [ ] **APIs & Services → OAuth consent screen**:
  - User Type: External
  - App name: `Datapp`
  - User support email + developer contact: your email
  - Authorized domains: `datapp.com.ar`
...
- [ ] **Credentials → + Create credentials → OAuth client ID**:
  ...
  - Name: `datapp-admin`
  - **Authorized JavaScript origins**: `https://datapp.com.ar`
  - **Authorized redirect URIs**: `https://datapp.com.ar/api/auth/callback/google`
...
- [ ] **First sign-in**: after the first deploy is up, hit `https://datapp.com.ar/login` and click **"Continuar con Google"**. The first sign-in matching `OWNER_EMAIL` auto-creates (or migrates) the `super_admin` row. Subsequent users have to be created from `/users/new`.
...
> Generate every secret on a machine that will never see prod. Paste them into Dokploy → Environment Variables for the datapp stack. **Never commit a real value to the repo.**
...
- [ ] Sentry DSNs (one per project — `datapp-api` and `datapp-admin`).
...
```env
DATABASE_URL=postgresql://datapp:<pw>@postgres:5432/datapp
DIRECT_URL=postgresql://datapp:<pw>@postgres:5432/datapp
REDIS_URL=redis://redis:6379/0
AUTH_SECRET=...
JWT_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\n...\n-----END PRIVATE KEY-----"
JWT_PUBLIC_KEY="-----BEGIN PUBLIC KEY-----\n...\n-----END PUBLIC KEY-----"
ENCRYPTION_MASTER_KEY=...
APP_URL_API=https://api.datapp.com.ar
APP_URL_ADMIN=https://datapp.com.ar
...
- [ ] In Dokploy, hit **Deploy** on the datapp application. The first build pulls images for postgres + redis and builds api / worker / admin from the Dockerfiles in `infra/docker/`.
...
- [ ] Seed an initial super_admin user. Run a one-shot command in the api container:
  ...
  docker compose -f infra/dokploy/docker-compose.dokploy.yml exec api \
    pnpm --filter @datapp/db seed
  ```

  Or, if no seed script fits, log into the DB and insert manually with an argon2id hash generated by the auth module.

- [ ] Hit `https://datapp.com.ar` — login as the super_admin, immediately enroll 2FA (it's required for admin/super_admin roles when `FEATURE_2FA_ENFORCED=true`), download the recovery codes, store them somewhere safe.

- [ ] Smoke checks:

  ```sh
  curl -sf https://api.datapp.com.ar/v1/health/live
  curl -sf https://api.datapp.com.ar/v1/health/ready | jq
  ```
...
docker compose -f infra/dokploy/docker-compose.dokploy.yml \
  pull datapp-api:<previous-sha> datapp-admin:<previous-sha> datapp-api-worker:<previous-sha>
...
- **`POSTGRES_PASSWORD`**: change via `ALTER USER datapp WITH PASSWORD '...'`, then update `DATABASE_URL` and `DIRECT_URL` in Dokploy and redeploy.
...
| Symptom                                | First check                                                                 |
| -------------------------------------- | --------------------------------------------------------------------------- |
| 502 from `api.datapp.com.ar`         | `docker logs datapp-api`. Liveness probe failing? Migration failed on boot?    |
| 503 on `/v1/health/ready`              | DB or Redis unreachable. Check `datapp-postgres` / `datapp-redis` containers.     |
